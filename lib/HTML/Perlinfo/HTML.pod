...
You can capture the HTML output by assigning it to a scalar. Then you can alter the HTML before printing it or doing something else with it. Here is an example that uses the L<perlinfo|HTML::Perlinfo#perlinfo> function from L<HTML::Perlinfo>:  
...
These object attributes allow you to change the HTML CSS settings to achieve a stylish effect. When using them, you must pass them a parameter. Please see your favorite HTML guide for acceptable CSS values. Refer to the HTML source code of the page for the defaults.
...
You can request a content-type header. But you don't have to, since perlinfo is smart enough to know if you are using it over http or not. What's more, perlinfo uses the header function from Lincoln Stein's CGI module to produce the header. This function has the advantage of mod_perl detection (Thanks Lincoln). To send a header, set header to 1 in the constructor. Observe:

    	$p = HTML::Perlinfo->new(
		    header  => 1   # Change value to 0 to remove the header
	    );
...
The number 1 turns on all default links (and the camel images if applicable) and 0 will remove them.

For example, to remove the default links and images in the info_all method, you would say:
	
	$p->info_all( links=>[0] ); # contains no images or links. Good for printing!
...
For more information on L<print_modules|HTML::Perlinfo::Modules#print_modules> and its L<link|HTML::Perlinfo::Modules#link> parameter, please see L<HTML::Perlinfo::Modules>. 
